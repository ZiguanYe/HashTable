...
struct LinearProber : public Prober {
    ...
    HASH_INDEX_T next() 
    {
        ...
        else
        {
            ...
            numProbes_++;
            //std::cout << "this is runned ---" << std::endl;
            return loc;
        }
    }
};
...
private:
    ...
    //total number of keys in the hash table
    int numKeys;
    int numInserts;
    double rAlpha_;
};
...
{
    //double fac = (double)numKeys/CAPACITIES[mIndex_];
    double fac = (double)numInserts/CAPACITIES[mIndex_];
    //std::cout<< "lets just see what is fac over here " << fac << std::endl;
    
    if(fac >= rAlpha_)
    {
        resize();
        //move on to the next prime number size
    }
    
    HASH_INDEX_T idx = probe(p.first);
    if(idx == npos)
    {
        throw std::logic_error("There are no free location.");
    }
    else if(internalFind(p.first)!=nullptr)
    {
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    std::vector<HashItem*> tmpHt;
    
    for(size_t i=0; i<table_.size(); i++)
    {
        if(table_[i]!=nullptr && table_[i]->deleted!=true)
        {
            HashItem* tmp = new HashItem(table_[i]->item);
            tmpHt.push_back(tmp);
        }
        delete table_[i];
        table_[i] = nullptr;
    ...
    for(size_t i=CAPACITIES[mIndex_]; i<CAPACITIES[mIndex_+1]; i++)
    {
    ...
    for(size_t i=0; i<tmpHt.size(); i++)
    {
        //seg fault
        this->insert(tmpHt[i]->item);
        delete tmpHt[i];
    }
...
HASH_INDEX_T HashTable<K,V,Prober,Hash,KEqual>::probe(const KeyType& key) const
{
    ...
    while(Prober::npos != loc)
    {
        ...
        //&& kequal_(key, table_[loc]->item.first
        else if(table_[loc]->deleted == false && kequal_(key, table_[loc]->item.first)){
            return loc;
        }
